...
A distribuição F é uma razão de qui-quadrados:

$$
F = \frac{\frac{X_u^2}{u}}{\frac{X_v^2}{v}} \quad \rightarrow \quad F \quad \sim \quad F_{\frac{u}{v}}
$$

Para simular valores de uma distribuição $\mathbf{F}$, basta simular valores de duas qui-quadrados e fazer a razão entre elas:
...
Distribution F da razao de variancias de de variaveis normales de uma ima populacao $\sigma^2 \quad \sim \quad \frac{\sigma}{k-1}X^2_{k-1}$
...
What if we deviate from normality? Let's test with the Weibull distribution, using two samples with the same parameter, so that:
...
for (i in 1:iter)
  {
  weibull1 <- rweibull(n1, shape = 2, scale = 20)
  weibull2 <- rweibull(n2, shape = 2, scale = 20)
